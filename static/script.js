...
async function playLiveAudio(text, language) {
    ...
    const audio = document.getElementById('live-voice-audio');
...
document.getElementById('translate-speak').addEventListener('click', async () => {
    ...
    showStatus('translate-status', 'Generating audio...', 'info');
    try {
        const payload = await requestJson('/api/speak', {
            method: 'POST',
            body: JSON.stringify({ text, language }),
        });
        const audio = document.getElementById('translate-audio');
        audio.src = payload.audio_url;
        audio.classList.remove('hidden');
        await audio.play();
        showStatus('translate-status', 'Audio ready.', 'success');
    } catch (error) {
        showStatus('translate-status', error.message, 'error');
    }
});
...
document.getElementById('voice-translate').addEventListener('click', async () => {
    ...
    try {
        ...
        updateStats(payload.stats);
        const speechPayload = await requestJson('/api/speak', {
            method: 'POST',
            body: JSON.stringify({ text: payload.translated_text, language: targetLang }),
        });
        const audio = document.getElementById('voice-audio');
        audio.src = speechPayload.audio_url;
        audio.classList.remove('hidden');
        await audio.play();
        showStatus('voice-status', 'Voice translation complete.', 'success');
        await refreshHistory();
    } catch (error) {
